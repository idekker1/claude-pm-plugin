...
### (d) TEST SPEC (soft gate)

Applies to job types: `implementation`, `schema-change`, `refactor`.
Skip for: `test`, `docs-update`, `review`.

Check whether the user's request contains enough behavioral description to produce
a test spec — observable inputs, outputs, invariants, and error cases.

**If a spec can be derived** (from the request or the matching ROADMAP.md item):
- Extract it and include a `## Test spec` section in the job file (STEP 3)
- Write it in behavioral terms only: what goes in, what comes out, what must always hold
- Do NOT describe internal functions or implementation structure

**If no spec can be derived** and the job is non-trivial:
- Ask before writing the job file:
  > "What should be tested? Describe the expected behavior from the outside: what inputs
  > produce what outputs, what error cases must be handled, what invariants must always hold."
- Once answered, include the spec in the job file

**Waiver** — if the job is trivial or cosmetic (config rename, text change, no observable
behavior change): proceed without a test spec. Note the waiver in the Notes section:
  > "Test spec waived: no observable behavior change."

This is a **soft gate** — if the user declines to provide a spec, create the job and note:
  > "No test spec provided. Auto-test generation will be skipped when this job completes."
...
For `implementation`, `schema-change`, `refactor` jobs:

If a test spec was collected in STEP 2(d), include it:
```markdown
## Test spec
- **Invariants:** [...]
- **Inputs to test:** [...]
...
If a waiver was granted or the user declined: omit the section entirely (its absence
signals run-jobs to skip auto-test-job creation).
...
- **Open issues addressed**: list issue IDs from open.md that this job will resolve,
  or "none"
- **Test spec**: "included" | "waived (cosmetic)" | "skipped (user declined — auto-test will not run)"
